...
def review_management():
    if request.method == "POST":
        ...
        db.session.commit()
        return redirect(url_for('woman_identify', woman_id=woman_id))
...
# search for data by name of the women
...
def search():
    if request.method == 'POST':
        ...
        women_info = Women.query.filter(Women.name.like(search)).all()
        return render_template('homepage.html', women_info=women_info , pageTitle='Powerful women in Tech', legend="Search Results")
    else:
        return redirect("/")

#  filter the data by the position of the women
@app.route("/filter", methods=["GET", "POST"])
def filter_data():
    if request.method == "POST":
        filter_value = request.form["filter_value"]
        filtered_info = Women.query.filter(Women.position == filter_value).all()
        women_info = Women.query.filter(Women.position.like(filter_value)).all()
        return render_template("homepage.html",filtered_info=filtered_info,women_info=women_info)
    else:
        return redirect("/")
